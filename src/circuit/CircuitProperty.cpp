...
void
PropertyManager::collectClassificationItem(const API_Guid& itemGuid, GS::Array<API_Guid>& outGuids)
{
...
GSErrCode
PropertyManager::ensureGroup()
{
  if (s_groupGuid != APINULLGuid)
    return NoError;
  ...
  s_groupGuid = group.guid;
  return NoError;
}

GSErrCode
PropertyManager::ensureDefinition()
{
  if (s_propertyGuid != APINULLGuid)
    return NoError;
  ...
  err = ACAPI_Property_GetPropertyDefinitions(s_groupGuid, allDefinitions);
  if (err != NoError)
    return err;

  for (const API_PropertyDefinition& existing : allDefinitions) {
    if (existing.name == PROPERTY_NAME) {
      s_propertyGuid = existing.guid;
      return NoError;
    }
  }

  API_PropertyDefinition definition;
  definition.groupGuid = s_groupGuid;
  definition.name = PROPERTY_NAME;
  ...
  s_propertyGuid = definition.guid;
  return NoError;
}

GS::UniString
PropertyManager::generateId()
{
  GS::Guid gsGuid;
  gsGuid.Generate();
...
GS::UniString
PropertyManager::getId(const API_Guid& elemGuid)
{
  if (ensureDefinition() != NoError)
    return GS::UniString();

  API_Property property = {};
  if (ACAPI_Element_GetPropertyValue(elemGuid, s_propertyGuid, property) != NoError)
    return GS::UniString();
...
GSErrCode
PropertyManager::setId(const API_Guid& elemGuid, const GS::UniString& circuitId)
{
  ...
  API_Property property = {};
  property.definition.guid = s_propertyGuid;
