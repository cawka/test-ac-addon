...
API_AddParType* GdlWireElement::FindParam (API_ElementMemo& memo, const char* paramName)
{
...
bool GdlWireElement::GetParam (const API_ElementMemo& memo, const char* paramName, double& outValue)
{
...
bool GdlWireElement::SetParam (API_ElementMemo& memo, const char* paramName, double value)
{
...
void GdlWireElement::FillFixedUniBuffer (GS::uchar_t* dest, USize destCapacity, const GS::UniString& name)
{
...
API_Guid GdlWireElement::Create (const API_Coord& startPoint, const API_Coord& endPoint, short layerIndex)
{
	API_LibPart libPart = {};
	...
	if (ACAPI_LibraryPart_GetParams (libPart.index, &libA, &libB, &addParNum, &addPars) == NoError && addPars != nullptr) {
		if (memo.params != nullptr)
...
GSErrCode GdlWireElement::SetEndpoint (const API_Guid& wireGuid, WireEnd movedEnd, const API_Coord& newPoint)
{
	API_Element element = {};
...
bool GdlWireElement::IsInstance (const API_Guid& elemGuid)
{
	API_Element element = {};
	...
	return GS::UniString (libPart.docu_UName) == kLibPartName;
}
