...
API_Guid
SplineWireElement::create(const std::vector<WireNode>& nodes, short layerIndex)
{
  if (nodes.size() < 2)
    return APINULLGuid;
...
GSErrCode
SplineWireElement::setNodes(const API_Guid& wireGuid, const std::vector<WireNode>& nodes)
{
  if (nodes.size() < 2)
...
GSErrCode
SplineWireElement::setEndpoint(const API_Guid& wireGuid, WireEnd end, const API_Coord& newPoint)
{
  API_Element element = {};
  element.header.guid = wireGuid;
...
bool
SplineWireElement::isInstance(const API_Guid& elemGuid)
{
  API_Elem_Head head = {};
  head.guid = elemGuid;
